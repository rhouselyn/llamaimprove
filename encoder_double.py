...
class PatchMLPEncoder(nn.Module):
    ...
    2. ReLU 激活（使用 ReLU）
    3. Linear(hidden_dim, 1) - 映射到中间值
    4. Flatten 到 32 维
    5. 无参数 LayerNorm（新增，激活前）
    6. ReLU 激活（新增）
    7. Linear(32, 32) - 最终映射（新增）
    8. LayerNorm（有参数，elementwise_affine=True）

    对于 16×16 patch：
    - 经过 (4,2) conv 后得到 4×8 的 feature map
    - 每个位置有 hidden_dim 个特征
    - Flatten 后得到 32 维向量
    - 通过 LayerNorm (无参数) + ReLU + Linear(32,32) 映射到最终 32 维编码
    - 应用 LayerNorm (有参数)
    """
    ...
    def __init__(self, hidden_dim=24, learnable=True):
        ...
        # 新增：最终线性层 (32 -> 32)
        self.final_linear = nn.Linear(32, 32, bias=False)

        # 新增：ReLU 激活 for 中间 32 维
        self.final_activation = nn.ReLU()

        # 新增：中间无参数 LayerNorm（激活前，elementwise_affine=False）
        # self.intermediate_norm = nn.LayerNorm(32, elementwise_affine=False)

        # Layer Normalization（应用在最终输出，有参数）
        self.norm = nn.LayerNorm(32, elementwise_affine=True)  # encoding_size = 32
        ...
        with torch.no_grad():
            ...
            # 新增：final_linear 使用 Xavier 初始化
            nn.init.xavier_uniform_(self.final_linear.weight)
            # 最终 norm 初始化（gamma=1, beta=0）
        ...
        self.final_linear.weight.requires_grad = learnable
        if not learnable:
            # 中间 norm 无参数，无需冻结
            # 最终 norm 有参数，冻结
    ...
    def forward(self, x):
        ...
        输出: [batch_size, encoding_size]  # encoding_size = 32
        """
        ...
        # 6. 新增：中间无参数 LayerNorm（激活前）
        # x = self.intermediate_norm(x)  # [batch_size, 32]

        # 7. 新增：ReLU 激活
        x = self.final_activation(x)  # [batch_size, 32]

        # 8. 新增：最终线性层 (32 -> 32)
        x = self.final_linear(x)  # [batch_size, 32]
...
class ImprovedLaplaceKLLoss(nn.Module):
    ...
    def __init__(self, weight_original=1.0, weight_count=0.5, weight_cov=0.5, distributed=False, mine_model=None):
        ...
        self.target_binom_mean = 16.0  # Binomial mean
        # 新增：目标二项分布参数
    ...
    def forward(self, encodings, current_epoch=None, total_epochs=None):
        ...
        count_approx = binary_approx.sum(dim=1)  # [N], forward=pos_count (0~32), backward=sum(grad=1)
        ...
        if self.mine_model is not None:
            ...
            # 分割成左右16维
            left = encodings_perm[:, :16]  # [N, 16]
            right = encodings_perm[:, 16:]  # [N, 16]
            # 对右边的batch维度（行）打乱
...
def analyze_distribution(encoder, data_loader, device, n_batches=10,
    ...
    all_encodings_2d = np.concatenate(all_patch_outputs, axis=0)  # [total_patches, 32]，用于 cov/mi
    all_patch_outputs_np = all_encodings_2d.flatten()  # [total_patches * 32]，用于 hist/KS 等一维分布
    ...
    if mine_model is not None:
        ...
        left = encodings_perm[:, :16]
        right = encodings_perm[:, 16:]
        right_shuffle = right[torch.randperm(right.size(0))]
    ...
    print(f"  Mean: {all_positive_counts.mean():.4f} (target: 16.0)")
    print(f"  Std:  {all_positive_counts.std():.4f} (target: 4.0)")  # 注意：std目标仍是sqrt(32*0.5*0.5)=4
    print(f"  Min:  {all_positive_counts.min()}")
    print(f"  Max:  {all_positive_counts.max()}")

    print(f"\nEstimated Count Statistics:")
    print(f"  Mean: {all_estimated_counts.mean():.4f} (target: 16.0)")
...
def plot_comparison(before_data, after_data, save_path, epoch_info=""):
    ...
    for idx, (phase, color, label) in enumerate(zip(phases, colors, phase_labels)):
        ...
        ax1.plot(x_binom, stats.binom.pmf(x_binom, encoding_size, 0.5), 'go-', linewidth=2, markersize=4,
                 label=f'Target Binomial(32, 0.5)', alpha=0.9)
        ax1.plot(x_binom, stats.binom.pmf(x_binom, encoding_size, p_positive), 'ro--', linewidth=2, markersize=3,
                 label=f'Actual Binomial(32, {p_positive:.3f})', alpha=0.8)
        ax1.axvline(16, color='green', linestyle='--', linewidth=2, alpha=0.5)
