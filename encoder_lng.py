...
class PatchMLPEncoder(nn.Module):
    ...
    def __init__(self, conv_kernel=2, hidden_dim=12, learnable=True):
        ...
        # Layer Normalization
        self.norm = nn.LayerNorm(64)  # encoding_size = 64
...
class ImprovedLaplaceKLLoss(nn.Module):
    ...
    def kl_gaussian(self, mu1, sigma1_sq, mu2, sigma2_sq):
        return 0.5 * (sigma1_sq / sigma2_sq + (mu1 - mu2)**2 / sigma2_sq - 1 + torch.log(sigma2_sq / sigma1_sq))
    ...
    def forward(self, encodings, current_epoch=None, total_epochs=None):
        ...
        # 计算每个维度的mu和sigma^2（用于原KL）
        mu = torch.mean(encodings, dim=0)  # [D]
        sigma_sq = torch.var(encodings, dim=0, unbiased=False)  # [D]
        sigma_sq = torch.clamp(sigma_sq, min=1e-6)  # 避免除零和log负

        # ===== 1. 原KL损失: D_KL(N(μ, σ^2) || N(0, 1)) =====
        kl_div_original = self.kl_gaussian(mu, sigma_sq, self.target_mean, self.target_var)
...
def analyze_distribution(encoder, data_loader, device, n_batches=10,
    ...
    estimated_std = np.std(all_patch_outputs_np)
    ...
    print(f"  Mean: {estimated_mu:.6f} (target: 0.0)")
    print(f"  Std: {estimated_std:.6f} (target: 1.0)")
...
def plot_comparison(before_data, after_data, save_path, epoch_info=""):
    ...
    for idx, (phase, color, label) in enumerate(zip(phases, colors, phase_labels)):
        ...
        estimated_std = np.std(patch_outputs)
