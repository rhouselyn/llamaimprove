...
class ImprovedGaussianKLLoss(nn.Module):
    """
    改进版损失函数：
    1. KL散度损失（编码分布与 N(0, 0.5) 高斯的KL散度）
    2. Cov 损失（协方差矩阵非对角元素平方均值，鼓励维度独立）
    3. 新增：基于STE的归一化维度均值KL损失（mean-1/2 归一化版本，KL到 N(0, 0.25)）
    """
    ...
    def forward(self, encodings, current_epoch=None, total_epochs=None):
        ...
        """
        # ===== 1. KL散度：编码分布 vs N(0, 0.5) =====
        mean = encodings.mean(dim=0)  # [encoding_size]
        var = encodings.var(dim=0, unbiased=False)  # [encoding_size]

        # KL散度: D_KL(N(μ, σ²) || N(0, target_var)) = log(target_var / σ²)/2 + (σ² + μ²)/(2*target_var) - 1/2
...
def main(args):
    ...
    logger.info(f"Loss weights - Original KL: {args.weight_original}, Count KL: {args.weight_count}, Cov: {args.weight_cov}")
