...
class ImprovedLaplaceKLLoss(nn.Module):
    ...
    2. Cov 损失（协方差矩阵非对角元素平方均值，鼓励维度独立） - 只作为观测，不计入损失
    ...
    def __init__(self, weight_original=1.0, weight_count=0.5, weight_cov=0.5, distributed=False, mine_model=None):
        ...
        self.weight_cov = weight_cov  # Cov损失权重（未使用于损失，仅观测）
        self.distributed = distributed
        self.mine_model = mine_model  # Pre-trained MINE model for MI observation
    ...
    def forward(self, encodings, current_epoch=None, total_epochs=None):
        ...
        # ===== 2. Cov损失：非对角协方差接近0（只作为观测，不计入总损失） =====
        ...
        # ===== 3. 基于STE的归一化KL损失（或Binom NLL，根据epoch切换） =====
        ...
        if current_epoch is not None and current_epoch <= 2:
            # 前2个epoch：使用原高斯KL
            # 原计算：基于mean/2和标准化
        ...
        # ===== MINE-based MI estimation (only as observation, if model provided) =====
        ...
        stats_dict = {
            ...
            'mi_value': mi_value.item(),  # MINE MI estimate as observation
            # 真实统计
...
def analyze_distribution(encoder, data_loader, device, n_batches=10,
    ...
    # 计算Laplace参数估计
    estimated_mu = np.median(all_patch_outputs)
    estimated_b = np.mean(np.abs(all_patch_outputs - estimated_mu))
    ...
    print(f"  Std:    {all_patch_outputs.std():.6f} (target: {np.sqrt(2) * 1.0:.6f})")
    print(f"  |x|_avg: {np.abs(all_patch_outputs).mean():.6f} (target: 1.0)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weight-cov", type=float, default=1.0, help="Weight for covariance loss")
    # Save args
