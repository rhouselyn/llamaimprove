...
class GaussianKLLoss(nn.Module):
    ...
    def __init__(self, weight_original=1.0, weight_count=1.0, distributed=False):
        ...
        self.target_cdf = torch.tensor(stats.binom.cdf(np.arange(self.encoding_size + 1), self.encoding_size, 0.5), dtype=torch.float32)
    ...
    def forward(self, encodings):
        ...
        # 计算Wasserstein-1距离
        avg_cdf = torch.cumsum(avg_pmf, dim=0)
        target_cdf = self.target_cdf.to(encodings.device)
        kl_loss_count = torch.abs(avg_cdf - target_cdf).sum()
