...
class EntropyPenaltyLoss(nn.Module):
    """
    LFQ中的熵罚项损失：ℒ_entropy = 𝔼[H(q(z))] - H[𝔼(q(z))]

    - 假设encodings是logits-like，使用sigmoid得到软概率p
    - 计算Bernoulli熵（nats单位）
    """
    ...
    def forward(self, encodings):
        ...
        """
        # encodings视为logits，软概率p = sigmoid(encodings)
        probs = torch.sigmoid(encodings)  # [N, D], N = total_patches, D = encoding_size

        # 计算熵函数 H(p) = -p log p - (1-p) log (1-p)
        # 添加epsilon避免log(0)
        eps = 1e-10
        per_dim_ent = -probs * torch.log(probs + eps) - (1 - probs) * torch.log(1 - probs + eps)  # [N, D]

        # 第一项: 𝔼[H(q(z))] = 平均每个样本的熵和
        per_sample_ent = per_dim_ent.sum(dim=1)  # [N], 每个patch的总熵
        avg_per_sample_ent = per_sample_ent.mean()  # scalar

        # 第二项: H[𝔼(q(z))] = 平均概率的熵和
        avg_probs = probs.mean(dim=0)  # [D]
        global_ent = -avg_probs * torch.log(avg_probs + eps) - (1 - avg_probs) * torch.log(1 - avg_probs + eps)  # [D]
        global_ent = global_ent.sum()  # scalar

        # 罚项损失
        total_loss = avg_per_sample_ent - global_ent

        # 统计信息
        pos_counts = (encodings > 0).sum(dim=1).float()  # 真实正值个数

        stats_dict = {
            'total_loss': total_loss.item(),
            # 熵相关
            'avg_per_sample_ent': avg_per_sample_ent.item(),
            'global_ent': global_ent.item(),
            # 真实统计
            ...
            'output_var': encodings.var().item(),
            'output_std': torch.sqrt(encodings.var()).item(),
            # 整体统计
...
def analyze_distribution(encoder, data_loader, device, n_batches=10,
    ...
    return all_positive_counts, all_patch_outputs
...
def plot_comparison(before_data, after_data, save_path, epoch_info=""):
    ...
    """
    fig = plt.figure(figsize=(20, 12))  # 调整高度，因为少一行
    gs = fig.add_gridspec(3, 4, hspace=0.3, wspace=0.3)  # 改为3行
    ...
    for batch_idx, (images, _) in enumerate(data_loader):
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            logger.info(
                ...
                f"Loss: {avg_loss:.6f} | "
                f"Avg_per_sample_ent: {avg_stats['avg_per_sample_ent']:.6f} | "
                f"Global_ent: {avg_stats['global_ent']:.6f} | "
...
def main(args):
    ...
    if rank == 0:
        ...
        logger.info(f"Using entropy penalty loss from LFQ")
    else:
        experiment_dir = None
    ...
    # Create loss（新损失）
    criterion = EntropyPenaltyLoss().to(device)
