...
def analyze_distribution(encoder, data_loader, device, n_batches=10,
    ...
    all_patch_outputs_np = np.concatenate(all_patch_outputs, axis=0)
    all_patch_outputs = torch.from_numpy(all_patch_outputs_np).to(device)  # 转换为Torch用于计算
    ...
    # ===== 计算 cov_loss（只在这里计算） =====
    cov_matrix = torch.cov(all_patch_outputs.T)  # [encoding_size, encoding_size]
    ...
    if mine_model is not None:
        N, D = all_patch_outputs.shape
        binary_values = (all_patch_outputs > 0).float()  # 无STE，因为是观察
    ...
    if save_data is not None:
        save_data[phase] = {
            ...
            'patch_outputs': all_patch_outputs_np,
