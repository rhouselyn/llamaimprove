...
from tokenizer.tokenizer_image.bq_model import VQModel, ModelArgs  # 调整为前缀tokenizer.tokenizer_image
...
def main(args):
    ...
    # Set shared seed for model initialization (ensure all ranks have same weights)
    ...
    # Now set unique seed for per-rank randomness (data loading, etc.)
    ...
    if args.vq_ckpt is None:
        # Define separate transform for ZCA accumulation (no random augmentation)
        zca_transform = transforms.Compose([
            transforms.Resize(args.image_size),  # Resize to image_size
            transforms.CenterCrop(args.image_size),  # Center crop
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)
        ])
        zca_dataset = build_dataset(args, transform=zca_transform)
        zca_sampler = DistributedSampler(
            zca_dataset,
            num_replicas=dist.get_world_size(),
            rank=rank,
            shuffle=False,  # Deterministic for stats
        ...
        zca_loader = DataLoader(
            ...
            pin_memory=True,
            drop_last=False  # Don't drop for full accumulation
        )
        ...
        if args.zca_ckpt is not None:
            ...
            # Broadcast ZCA params to all processes
            zca = vq_model.encoder.zca
            ...
            zca_sampler.set_epoch(0)  # Use epoch 0 for accumulation
            vq_model.encoder.eval()  # Encoder in eval mode for accumulation
            accum_steps = 0
            total_samples = 0
            accum_start_time = time.time()
            progress = tqdm(zca_loader, desc="Accumulating ZCA stats") if rank == 0 else zca_loader
            for x, y in progress:
                x = x.to(device)
                with torch.no_grad():
                ...
                if accum_steps % args.log_every == 0 and rank == 0:
    ...
    if args.vq_ckpt:
        ...
        if not args.finetune:
            ...
            start_epoch = int(train_steps / int(len(zca_dataset) / args.global_batch_size))
            train_steps = int(start_epoch * int(len(zca_dataset) / args.global_batch_size))
        else:
            train_steps = 0
            start_epoch = 0
        del checkpoint
    ...
    # Setup data for training (with random augmentation):
