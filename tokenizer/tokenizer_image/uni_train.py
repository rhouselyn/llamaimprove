...
class PCAWhitening(nn.Module):  # 修改：ZCA -> PCA，用于临时积累stats
    ...
    def compute_whitening_matrix(self):
        ...
        new_matrix = torch.mm(eigenvectors, torch.diag(1.0 / sqrt_eigenvalues))  # 修改：移除 @ eigenvectors.t()
...
class IntegratedModel(nn.Module):
    ...
    def accumulate_and_fuse_pca(self, loader, device, rank, world_size, checkpoint_dir, logger):
        ...
        temp_pca = PCAWhitening(dim).to(device)
        ...
        if rank == 0:
            ...
            logger.info("Computed and fused PCA into CSQEncoder linear layer.")
    ...
    #     if self.quantize.l2_norm:
    #         h = F.normalize(h, p=2, dim=-1)  # 新增：确保 flatten 后也 norm
...
def joint_train_step(model, vq_loss, index_loss_fn, x, joint_optimizer, optimizer_disc, scaler, scaler_disc, args, train_steps, device, logger):
    ...
    logits = model.module.fc_classifier(quant_h.permute(0, 2, 3, 1).reshape(-1, 8))  # 注意：classify_forward 已内含 norm
    index_loss = index_loss_fn(logits, index.view(-1))

    post_quant_z = model.module.post_quant_conv(quant_h)
...
def decoder_train_step(model, vq_loss, x, decoder_optimizer, optimizer_disc, scaler, scaler_disc, args, train_steps, device, logger):
    ...
    if args.max_grad_norm != 0.0:
        ...
        torch.nn.utils.clip_grad_norm_(chain(model.module.decoder.parameters(), model.module.quantize.embedding.parameters()), args.max_grad_norm)
    ...
    # 修改：返回三个loss，CE=0
    return loss_gen.item(), 0.0, loss_disc.item()
...
def main(args):
    ...
    if args.pca_ckpt:
        if rank == 0:
            ...
            logger.info("Loaded and fused PCA checkpoint into CSQEncoder linear layer.")
        dist.barrier()
    ...
    decoder_optimizer = torch.optim.Adam([
        ...
        {'params': model.quantize.embedding.parameters(), 'lr': args.lr / 10}
        ...
        for x, _ in loader:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                if epoch < args.distill_epochs:
                    ...
                    if not switched_to_decoder:
                        with torch.no_grad():
                            model.module.quantize.embedding.weight.copy_(model.module.fc_classifier.weight)
                            if model.module.quantize.l2_norm:
                                model.module.quantize.embedding.weight.data = F.normalize(model.module.quantize.embedding.weight.data, p=2, dim=-1)
                        model.module.quantize.embedding.weight.requires_grad = True
                        logger.info("Switched to decoder training with new codebook (unfrozen, low lr).")
                        switched_to_decoder = True
                    gen_l, ce_l, disc_l = decoder_train_step(model, vq_loss, x, decoder_optimizer, optimizer_disc, scaler, scaler_disc, args, train_steps, device, logger)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pca-ckpt", type=str, default='/mnt/afs/zhengmingkai/raozf/llamagen/results_integrated/031-Integrated_Model/checkpoints/pca.pt', help="Path to precomputed PCA checkpoint")
    parser.add_argument("--distill-epochs", type=int, default=20, help="Epochs for joint training std encoder and decoder")  # 修改为5
