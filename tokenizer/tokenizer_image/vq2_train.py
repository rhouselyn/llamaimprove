...
from tokenizer.tokenizer_image.vq2_model import compute_avg_min_distance  # 新增导入：用于计算平均最小间隔
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        # Switch to stage 2 after 20 epochs: detach quant, freeze encoder/codebook
        if epoch >= 20:
            vq_model.module.quantize.detach_quant = True
            if rank == 0:
                logger.info(f"Switching to stage 2 at epoch {epoch}: detaching quant, freezing encoder and codebook.")
        ...
        for x, y in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                if rank == 0:  # 只在rank=0计算和打印平均最小间隔
                    avg_min_dist = compute_avg_min_distance(vq_model.module.quantize)  # 新增：计算平均最小间隔
                    logger.info(
                        f"(step={train_steps:07d}) Train Loss: {avg_loss:.4f}, Train Steps/Sec: {steps_per_sec:.2f}, Avg Min Distance: {avg_min_dist:.4f}")
