...
from tokenizer.tokenizer_image.fsq_model import VQ_models
from tokenizer.tokenizer_image.vq_loss import VQLoss
from tokenizer.tokenizer_image.fsq_model import compute_avg_min_distance  # 平均最小间隔监控
...
def main(args):
    """
    Trains a new model (single-stage, STE-only).
    """
    ...
    if args.quantizer == "fsq":
        if args.fsq_bins.strip():
            fsq_bins_parsed = [int(x) for x in args.fsq_bins.split(",")]
        else:
            fsq_bins_parsed = [2] * args.codebook_embed_dim
        # 日志提示
        if rank == 0:
            logger.info(f"FSQ bins: {fsq_bins_parsed} (product={eval_product(fsq_bins_parsed)})")
    else:
        fsq_bins_parsed = None
    ...
    # DDP
    vq_model = DDP(vq_model.to(device), device_ids=[args.gpu] if hasattr(args, "gpu") else [device])
    vq_model.train()
    if args.ema:
        ema.eval()
    vq_loss = DDP(vq_loss.to(device), device_ids=[args.gpu] if hasattr(args, "gpu") else [device])
    vq_loss.train()
    ...
    logger.info(f"Training for {args.epochs} epochs (single stage, STE).")
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                if rank == 0:
                    ...
                    logger.info(
                        ...
                        f"Steps/Sec: {steps_per_sec:.2f}, Avg Min Distance: {avg_min_dist:.4f}"
                    )
                running_loss = 0
                log_steps = 0
                start_time = time.time()
            ...
            if train_steps % args.ckpt_every == 0 and train_steps > 0:
                if rank == 0:
                    model_weight = vq_model.module._orig_mod.state_dict() if args.compile else vq_model.module.state_dict()
                    checkpoint = {
                        "model": model_weight,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--aoss-bucket", type=str, default="imagenet", help="AOSS bucket name (only for aoss_imagenet dataset)")

    parser.add_argument("--image-size", type=int, choices=[256, 512], default=256)
    parser.add_argument("--epochs", type=int, default=40)  # 单阶段总 40 轮
    ...
    # === 新增：量化器选择与 FSQ 分箱 ===
    parser.add_argument("--quantizer", type=str, choices=["fsq", "vq"], default="fsq",
                        help="Choose FSQ (finite scalar quantization) or the original VQ.")
    parser.add_argument("--fsq-bins", type=str, default="2,2,3,4,4,4,4,5",
                        help="Comma-separated bin counts per dim for FSQ. Must match codebook_embed_dim length.")
