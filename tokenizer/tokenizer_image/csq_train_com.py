...
# requires_grad from utils.ema is useful, so we keep it
from utils.ema import requires_grad
from dataset.augmentation import random_crop_arr
from dataset.build import build_dataset
from tokenizer.tokenizer_image.csq_model import VQ_models
from tokenizer.tokenizer_image.csq_loss import VQLoss
...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/{experiment_index:03d}-{model_string_name}-DecoderOnly"  # Create an experiment folder
    ...
    # --- Freeze Encoder (no loading, just use init) ---
    logger.info("Freezing encoder parameters...")
    requires_grad(vq_model.encoder, False)
    vq_model.encoder.eval()  # Put encoder in eval mode permanently

    logger.info(
        f"Encoder parameters frozen. Total VQModel Parameters: {sum(p.numel() for p in vq_model.parameters()):,}")
    logger.info(
        f"Trainable Decoder Parameters: {sum(p.numel() for p in vq_model.decoder.parameters() if p.requires_grad):,}")
    ...
    # Setup optimizer (ONLY for Decoder)
    ...
    if args.decoder_ckpt_path:
        checkpoint = torch.load(args.decoder_ckpt_path, map_location="cpu")
        # --- Load Decoder, Optimizers, and Discriminator ---
        vq_model.decoder.load_state_dict(checkpoint["model"])
        ...
        del checkpoint
        logger.info(f"Resume decoder training from checkpoint: {args.decoder_ckpt_path}")
    ...
    if args.compile:
        ...
        # We only compile the parts that will be trained
        vq_model.decoder = torch.compile(vq_model.decoder)
        # Note: vq_model.encoder is NOT compiled as it's frozen and in eval mode

    # DDP wraps the entire model, but only decoder params have requires_grad=True
    vq_model = DDP(vq_model.to(device), device_ids=[args.gpu])
    # Set model to train mode (this is correct, it will set decoder to train, encoder stays eval)
    vq_model.train()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                # vq_model forward: encoder runs with no_grad, quantizer and decoder run with grad
                recons_imgs, codebook_loss = vq_model(imgs)

                loss_gen = vq_loss(inputs=imgs,
            ...
            if args.max_grad_norm != 0.0:
                ...
                # Clip grads for decoder parameters only
                torch.nn.utils.clip_grad_norm_(vq_model.module.decoder.parameters(), args.max_grad_norm)
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                loss_disc = vq_loss(inputs=imgs,
                                    reconstructions=recons_imgs.detach(),
                                    optimizer_idx=1,
                                    global_step=train_steps + 1,
                                    logger=logger,
                                    log_every=args.log_every)
            ...
            if train_steps % args.log_every == 0:
                ...
                logger.info(
                    f"(Epoch {epoch}, step={train_steps:07d}) Train Loss: {avg_loss:.4f}, Train Steps/Sec: {steps_per_sec:.2f}")
        ...
        if rank == 0:
            if args.compile:
                # Get state dict from original compiled module
                model_weight = vq_model.module.decoder._orig_mod.state_dict()
            else:
                model_weight = vq_model.module.decoder.state_dict()

            checkpoint = {
                "model": model_weight,  # Only Decoder weights
            ...
            # Save "last" checkpoint (overwritten every epoch)
            last_checkpoint_path = f"{checkpoint_dir}/decoder_last.pt"
            torch.save(checkpoint, last_checkpoint_path)
            logger.info(f"Saved last checkpoint to {last_checkpoint_path}")

            # Save epoch-specific checkpoint
            epoch_checkpoint_path = f"{checkpoint_dir}/decoder_epoch_{epoch:03d}.pt"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--decoder-ckpt-path", type=str, default=None,
                        help="Path to a decoder checkpoint to resume training from.")
    parser.add_argument("--data-face-path", type=str, default=None, help="face datasets to improve vq model")
    parser.add_argument("--vq-model", type=str, choices=list(VQ_models.keys()), default="VQ-16")

    # --- New Model Args (must match encoder) ---
    parser.add_argument("--hidden-dim", type=int, default=24,
                        help="Hidden dimension for the PatchMLPEncoder (must match pretrained model)")
    parser.add_argument("--encoding-dim", type=int, default=16,
                        help="Dimension of the patch encoding (must match pretrained model)")

    # --- Removed Codebook Args ---
    ...
    # parser.add_argument("--codebook-weight", type=float, default=1.0, help="codebook loss weight for vector quantization") # Will be set to 0
    ...
    parser.add_argument("--results-dir", type=str, default="results_decoder")
    ...
    # parser.add_argument("--ckpt-every", type=int, default=5000) # Removed, saving per epoch
