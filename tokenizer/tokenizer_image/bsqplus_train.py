# train.py
# Modified: Added logging for Fixed Gaussian Projection confirmation
...
from tokenizer.tokenizer_image.bsqplus_model import VQ_models, compute_avg_min_distance
...
def main(args):
    ...
    vq_model = VQ_models[args.vq_model](
        ...
        num_bits=args.num_bits,  # e.g., 14
        codebook_embed_dim=args.codebook_embed_dim,  # e.g., 8
        codebook_l2_norm=args.codebook_l2_norm,  # e.g., True
        codebook_size=args.codebook_size,  # Ignored for BSQ
        commit_loss_beta=args.commit_loss_beta,  # Sign Loss
        entropy_loss_ratio=args.entropy_loss_ratio,  # Balance Loss
...
if __name__ == "__main__":
    ...
    # BSQ Arguments
    parser.add_argument("--quantizer", type=str, default="bsq", choices=["bsq", "fsq", "vq"])
    parser.add_argument("--num-bits", type=int, default=64, help="Number of bits for quantizer (Intermediate)")
    parser.add_argument("--codebook-embed-dim", type=int, default=8, help="Output projection dimension (Spherical)")
    parser.add_argument("--sample", action='store_true', help="Add standard Gaussian noise to quantized bits during training") # <--- [新增]
