...
# 新增：第一阶段训练步，只CE loss
def stage1_train_step(model, index_loss_fn, x, stage1_optimizer, scaler, args, train_steps, device):
    B, _, H, W = x.shape
    ...
    flattened_h = quant_h.permute(0, 2, 3, 1).reshape(-1, model.module.quantize.e_dim)  # [-1, 8]
    logits = model.module.fc_classifier(flattened_h)  # [-1, 16384]
    ce_loss = index_loss_fn(logits, min_encoding_indices)
    ...
    if args.max_grad_norm != 0.0:
        ...
        torch.nn.utils.clip_grad_norm_(chain(model.module.std_encoder.parameters(), model.module.quant_conv.parameters(),
                                             model.module.fc_classifier.parameters()), args.max_grad_norm)
...
def main(args):
    ...
    # 修改：创建两个optimizer，第一阶段只优化前半部分
    stage1_optimizer = torch.optim.Adam(chain(model.std_encoder.parameters(), model.quant_conv.parameters(),
                                              model.fc_classifier.parameters()), lr=args.lr, betas=(args.beta1, args.beta2))
    ...
    if args.resume:
        if rank == 0:
            ...
            # 修改：resume时加载joint_optimizer到joint_optimizer，stage1_optimizer忽略或手动处理（假设resume从第二阶段）
        ...
        for x, _ in loader:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                ...
                if epoch < args.stage1_epochs:
                    ce_l = stage1_train_step(model, index_loss_fn, x, stage1_optimizer, scaler, args, train_steps, device)
                    gen_l = 0.0
                    disc_l = 0.0
                else:
            ...
            if train_steps % args.log_every == 0:
                ...
                # 修改log：新增CE，如果第一阶段只log CE
                if epoch < args.stage1_epochs:
                    logger.info(f"(step={train_steps:07d}) Stage1 CE: {avg_ce:.4f}, Steps/Sec: {steps_per_sec:.2f}")
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pca-ckpt", type=str, default='/mnt/afs/zhengmingkai/raozf/llamagen/results_integrated/042-Integrated_Model/checkpoints/pca.pt', help="Path to precomputed PCA checkpoint")
    parser.add_argument("--epochs", type=int, default=80)
    parser.add_argument("--stage1_epochs", type=int, default=40)  # 新增：第一阶段epochs
    ...
    parser.add_argument("--disc-start", type=int, default=500000)
