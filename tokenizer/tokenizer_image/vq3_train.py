...
# ===== 使用新的 vq2_model（本回复下方给出文件）=====
from tokenizer.tokenizer_image.vq3_model import VQ_models, compute_avg_min_distance
...
def compute_stage_and_prob(step, s1, s2, s3):
    """
    根据全局 step 返回 (stage, replace_prob)
    stage: 1, 2, or 3
    replace_prob: 阶段2从0线性增到1，其它阶段为0或1
    """
    if step < s1:
        return 1, 0.0
    elif step < s1 + s2:
        # 线性增，从 (step == s1) 时 ~0，至 (step == s1+s2-1) 约 1
        p = (step - s1 + 1) / float(s2)
        if p < 0.0: p = 0.0
        if p > 1.0: p = 1.0
        return 2, p
    else:
...
def main(args):
    ...
    # 三阶段总步数（达到后提前停止）
    total_steps_target = args.stage1_steps + args.stage2_steps + args.stage3_steps
    last_stage = None

    logger.info(f"Training for up to {total_steps_target} steps (stage1={args.stage1_steps}, stage2={args.stage2_steps}, stage3={args.stage3_steps}) ...")
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            if train_steps >= total_steps_target:
                if rank == 0:
                    logger.info(f"Reached target total steps {total_steps_target}. Stopping training.")
                break

            # ====== 阶段调度 ======
            stage, replace_prob = compute_stage_and_prob(
                train_steps,
                args.stage1_steps,
                args.stage2_steps,
                args.stage3_steps
            )
            if stage != last_stage and rank == 0:
                logger.info(f"Switching to stage {stage} at step {train_steps}. (replace_prob={replace_prob:.4f})")
            last_stage = stage

            # 配置量化器模式（并在 stage3 冻结 encoder/quantizer）
            vq_model.module.quantize.configure(stage=stage, replace_prob=replace_prob)
            if stage == 3:
                # 只训练 decoder
                requires_grad(vq_model.module.encoder, False)
                requires_grad(vq_model.module.quantize, False)
                requires_grad(vq_model.module.quant_conv, False)
                requires_grad(vq_model.module.decoder, True)
                requires_grad(vq_model.module.post_quant_conv, True)
            else:
                # encoder/decoder 均可训练，quantizer 本身没有损失（参数不会被更新）
                requires_grad(vq_model.module.encoder, True)
                requires_grad(vq_model.module.quantize, False)
                requires_grad(vq_model.module.quant_conv, True)
                requires_grad(vq_model.module.decoder, True)
                requires_grad(vq_model.module.post_quant_conv, True)
            ...
            if train_steps % args.log_every == 0:
                ...
                if rank == 0:
                    # 计算平均最小间隔（注意：很耗显存/时间，可按需调低频率）
                    try:
                        # avg_min_dist = compute_avg_min_distance(vq_model.module.quantize)
                        avg_min_dist = 0

                        logger.info(
                            f"(step={train_steps:07d}) Stage={stage} p={replace_prob:.4f} | "
                            f"Train Loss: {avg_loss:.4f}, Steps/Sec: {steps_per_sec:.2f}, Avg Min Distance: {avg_min_dist:.4f}"
                        )
                    except Exception as e:
                        logger.info(
                            f"(step={train_steps:07d}) Stage={stage} p={replace_prob:.4f} | "
                            f"Train Loss: {avg_loss:.4f}, Steps/Sec: {steps_per_sec:.2f} (Avg Min Distance skipped: {e})"
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--entropy-loss-ratio", type=float, default=0.1, help="entropy loss ratio in codebook loss")
    ...
    # ===== 新增：三阶段步数（默认即 150k/100k/50k）=====
    parser.add_argument("--stage1-steps", type=int, default=150000, help="Stage 1 (AE) steps")
    parser.add_argument("--stage2-steps", type=int, default=100000, help="Stage 2 (mix replace) steps")
    parser.add_argument("--stage3-steps", type=int, default=50000,  help="Stage 3 (decoder-only, hard quant) steps")
