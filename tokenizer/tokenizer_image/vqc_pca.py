...
from tqdm import tqdm  # Added for progress in PCA accumulation
...
from tokenizer.tokenizer_image.vqc_model import VQ_models
...
# Added PCAWhitening class from the second script
class PCAWhitening(torch.nn.Module):
    def __init__(self, dim, eps=1e-5):
        super().__init__()
        self.dim = dim
        self.eps = eps
        self.sum_x = torch.nn.Parameter(torch.zeros(dim), requires_grad=False)
        self.sum_xxT = torch.nn.Parameter(torch.zeros(dim, dim), requires_grad=False)
        self.count = 0
        self.whitening_matrix = torch.nn.Parameter(torch.eye(dim), requires_grad=False)
        self.mean = torch.nn.Parameter(torch.zeros(dim), requires_grad=False)
    ...
    def compute_whitening_matrix(self):
        ...
        new_matrix = torch.mm(eigenvectors, torch.diag(1.0 / sqrt_eigenvalues))
        self.whitening_matrix.copy_(new_matrix)

    def forward(self, x):
        x_flat = x.view(-1, self.dim)
        x_centered = x_flat - self.mean
        x_whitened = torch.mm(x_centered, self.whitening_matrix)
...
def main(args):
    ...
    if args.pca_ckpt:
        if rank == 0:
            ...
            # Fuse into quant_conv
            if vq_model.quant_conv.bias is None:
                vq_model.quant_conv.bias = torch.nn.Parameter(torch.zeros(args.codebook_embed_dim, device=device))
            weight = vq_model.quant_conv.weight
            weight_matrix = weight.view(args.codebook_embed_dim, -1)  # [embed_dim, z_channels]
            new_weight_matrix = whitening_matrix.t() @ weight_matrix
            vq_model.quant_conv.weight.data = new_weight_matrix.view_as(weight)
            new_bias = - (mean @ whitening_matrix)
            vq_model.quant_conv.bias.data.copy_(new_bias)
            logger.info("Loaded and fused PCA checkpoint into quant_conv layer.")
        dist.barrier()
        # Broadcast fused parameters
        dist.broadcast(vq_model.quant_conv.weight, src=0)
        if vq_model.quant_conv.bias is not None:
            dist.broadcast(vq_model.quant_conv.bias, src=0)
        logger.info("Broadcasted fused quant_conv parameters.")
    else:
        # Accumulate stats with no augmentation (one epoch preprocessing)
        logger.info("Accumulating PCA stats in distributed mode with no augmentation...")
        ...
        dataset = build_dataset(args, transform=no_aug_transform)
        ...
        temp_pca = PCAWhitening(args.codebook_embed_dim).to(device)
        progress = tqdm(loader, desc="Accumulating PCA stats") if rank == 0 else loader
        for x, y in progress:
            imgs = x.to(device, non_blocking=True)
            with torch.no_grad():
                h = vq_model.encoder(imgs)  # Assume vq_model has .encoder
                quant_h = vq_model.quant_conv(h)  # Assume vq_model has .quant_conv
                flattened = quant_h.view(-1, args.codebook_embed_dim)
                temp_pca.update_stats(flattened)
        # All reduce stats
        ...
        if rank == 0:
            ...
            weight = vq_model.quant_conv.weight
            weight_matrix = weight.view(args.codebook_embed_dim, -1)  # [embed_dim, z_channels]
            new_weight_matrix = temp_pca.whitening_matrix.t() @ weight_matrix
            vq_model.quant_conv.weight.data = new_weight_matrix.view_as(weight)
            new_bias = - (temp_pca.mean @ temp_pca.whitening_matrix)
            vq_model.quant_conv.bias.data.copy_(new_bias)
            logger.info("Computed and fused PCA into quant_conv layer.")
            # Save PCA checkpoint
            pca_save = {
                "sum_x": temp_pca.sum_x.cpu(),
                "sum_xxT": temp_pca.sum_xxT.cpu(),
                "count": temp_pca.count,
                "whitening_matrix": temp_pca.whitening_matrix.cpu(),
                "mean": temp_pca.mean.cpu()
            }
            torch.save(pca_save, f"{checkpoint_dir}/pca.pt")
            logger.info(f"Saved PCA checkpoint to {checkpoint_dir}/pca.pt")
        dist.barrier()
        # Broadcast fused parameters
        dist.broadcast(vq_model.quant_conv.weight, src=0)
        if vq_model.quant_conv.bias is not None:
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--vq-ckpt", type=str, default='/mnt/afs/zhengmingkai/raozf/llamagen/results_tokenizer_image/008-VQ-16/checkpoints/0100000.pt', help="ckpt path for resume training")
    ...
    parser.add_argument("--codebook-size", type=int, default=17520, help="codebook size for vector quantization")
    ...
    # parser.add_argument("--results-dir", type=str, default="results_csq_image")
