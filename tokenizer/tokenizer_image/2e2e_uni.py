...
def stage1_train_step(model, vq_loss, index_loss_fn, x, joint_optimizer, optimizer_disc, scaler, scaler_disc, args, train_steps, device, logger):
    ...
    # 第一阶段：直接用quant_h作为z_q，不量化
    z_q = quant_h  # [B, 8, 16, 16]
        ...
        for x, _ in loader:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                if epoch < args.stage1_epochs:
                    gen_l, ce_l, disc_l = stage1_train_step(model, vq_loss, index_loss_fn, x, joint_optimizer, optimizer_disc, scaler, scaler_disc, args, train_steps, device, logger)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stage1_epochs", type=int, default=40, help="Number of epochs for stage 1 training")
