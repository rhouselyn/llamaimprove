...
class PatchMLPEncoder(nn.Module):
    ...
    结构：
    1. Conv2d(3, hidden_dim, kernel_size=(4,4), stride=(4,4)) - 每个 4×4 区域输出 hidden_dim 维特征
    2. Tanh 激活
    3. Linear(hidden_dim, 1) - 映射到最终编码值
    4. LayerNorm - 在最终输出上应用Layer Normalization

    对于 16×16 patch：
    - 经过 (4,4) conv 后得到 4×4 的 feature map
    - 每个位置有 hidden_dim 个特征
    - Flatten 后得到 16 × hidden_dim 维向量
    - 通过 Linear 层映射到 16 维编码
    - 应用 LayerNorm
    """
    ...
    def __init__(self, hidden_dim=24, learnable=True):
        ...
        # 第一层：卷积提取特征
        # 输入: [batch, 3, 16, 16]
        # 输出: [batch, hidden_dim, 4, 4]
        self.conv = nn.Conv2d(3, hidden_dim, kernel_size=(4,4), stride=(4,4), bias=False)

        # leakyrelu 激活函数
        self.activation = nn.LeakyReLU()

        # 第二层：线性映射到最终编码
        # 每个 4×4 位置的 hidden_dim 维特征独立映射到 1 维
        self.linear = nn.Linear(hidden_dim, 1, bias=False)

        # Layer Normalization
        self.norm = nn.LayerNorm(16, elementwise_affine=False)  # encoding_size = 16

        # 初始化权重
        with torch.no_grad():
            # 修改：conv 层使用 Xavier uniform 随机初始化（取代原来的常量填充）
            nn.init.xavier_uniform_(self.conv.weight)
            # Linear 层使用 Xavier 初始化（原样保留）
    ...
    def forward(self, x):
        ...
        输出: [batch_size, encoding_size]  # encoding_size = (4*4)=16
        """
        batch_size = x.shape[0]

        # 1. 卷积提取特征
        x = self.conv(x)  # [batch_size, hidden_dim, 4, 4]

        # 2. Tanh 激活
        x = self.activation(x)  # [batch_size, hidden_dim, 4, 4]

        # 3. 重排维度以便线性层处理
        # [batch_size, hidden_dim, 4, 4] -> [batch_size, 4, 4, hidden_dim]
        x = x.permute(0, 2, 3, 1)

        # 4. 线性映射：每个位置的 hidden_dim 维特征 -> 1 维
        x = self.linear(x)  # [batch_size, 4, 4, 1]

        # 5. 去除最后一维并展平
        x = x.squeeze(-1)  # [batch_size, 4, 4]
        x = x.flatten(1)  # [batch_size, 16]
...
class ImprovedLaplaceKLLoss(nn.Module):
    ...
    def __init__(self, weight_original=1.0, weight_count=0.5, weight_cov=0.5, distributed=False, mine_model=None):
        super().__init__()
        ...
        self.target_var = 1.0  # 标准高斯的目标方差
        self.target_count_mean = 0.0
        self.target_count_var = 0.25  # 1/4
        self.target_binom_mean = 8.0  # Binomial mean
        # 新增：目标二项分布参数
    ...
    def kl_gaussian(self, mu1, sigma21, mu2, sigma22):
        return 0.5 * (torch.log(sigma22 / sigma21) + (sigma21 + (mu1 - mu2) ** 2) / sigma22 - 1)
    ...
    def forward(self, encodings, current_epoch=None, total_epochs=None):
        ...
        # 计算每个维度的mu和sigma2（用于原KL）
        mu = torch.mean(encodings, dim=0)  # [D]
        sigma2 = torch.var(encodings, dim=0, unbiased=False)  # [D]
        sigma2 = torch.clamp(sigma2, min=1e-6)  # 避免log零或负

        # ===== 1. 原KL损失: D_KL(N(μ, σ²) || N(0, target_var)) =====
        kl_div_original = self.kl_gaussian(mu, sigma2, self.target_mean, self.target_var)
        ...
        count_approx = binary_approx.sum(dim=1)  # [N], forward=pos_count (0~16), backward=sum(grad=1)
        ...
        if self.mine_model is not None:
            ...
            # 分割成左右8维
            left = encodings_perm[:, :8]  # [N, 8]
            right = encodings_perm[:, 8:]  # [N, 8]
            # 对右边的batch维度（行）打乱
            ...
            mi_value += 1.6  # 添加指定的常数偏移

        # ===== 总损失（包括MI损失，条件化加入尾部惩罚，不包括cov_loss） =====
        total_loss = self.weight_original * kl_loss_original + (self.weight_cov * mi_value)
        ...
        stats_dict = {
            ...
            'output_sigma': torch.sqrt(sigma2).mean().item(),
...
def analyze_distribution(encoder, data_loader, device, n_batches=10,
    ...
    all_encodings_2d = np.concatenate(all_patch_outputs, axis=0)  # [total_patches, 16]，用于 cov/mi
    all_patch_outputs_np = all_encodings_2d.flatten()  # [total_patches * 16]，用于 hist/KS 等一维分布
    all_estimated_counts = np.array(all_estimated_counts)

    # 计算Gaussian参数估计
    estimated_mu = np.mean(all_patch_outputs_np)
    estimated_sigma = np.std(all_patch_outputs_np)
    ...
    if mine_model is not None:
        ...
        left = encodings_perm[:, :8]
        right = encodings_perm[:, 8:]
        right_shuffle = right[torch.randperm(right.size(0))]
        ...
        mi_value += 0.287396577  # 添加指定的常数偏移
    ...
    print(f"  Mean: {all_positive_counts.mean():.4f} (target: 8.0)")
    print(f"  Std:  {all_positive_counts.std():.4f} (target: 2.0)")  # 注意：std目标仍是sqrt(16*0.5*0.5)=2
    print(f"  Min:  {all_positive_counts.min()}")
    print(f"  Max:  {all_positive_counts.max()}")

    print(f"\nEstimated Count Statistics:")
    print(f"  Mean: {all_estimated_counts.mean():.4f} (target: 8.0)")
    print(f"  Std:  {all_estimated_counts.std():.4f} (target: 2.0)")
    ...
    print(f"  Median: {np.median(all_patch_outputs_np):.6f} (target: 0.0)")
    print(f"  Scale σ: {estimated_sigma:.6f} (target: 1.0)")
    print(f"  Std:    {all_patch_outputs_np.std():.6f} (target: 1.0)")
...
def plot_comparison(before_data, after_data, save_path, epoch_info=""):
    ...
    for idx, (phase, color, label) in enumerate(zip(phases, colors, phase_labels)):
        ...
        estimated_sigma = np.std(patch_outputs)
        ...
        ax1.plot(x_binom, stats.binom.pmf(x_binom, encoding_size, 0.5), 'go-', linewidth=2, markersize=4,
                 label=f'Target Binomial(16, 0.5)', alpha=0.9)
        ax1.plot(x_binom, stats.binom.pmf(x_binom, encoding_size, p_positive), 'ro--', linewidth=2, markersize=3,
                 label=f'Actual Binomial(16, {p_positive:.3f})', alpha=0.8)
        ax1.axvline(8, color='green', linestyle='--', linewidth=2, alpha=0.5)
        ...
        ax3.plot(x_range, stats.norm.pdf(x_range, loc=0, scale=1.0), 'g-', linewidth=2.5,
                 label='Target Gaussian(0,1.0)', alpha=0.9)
        ...
        ax3.grid(alpha=0.3)
        ks_stat, ks_p = stats.kstest(patch_outputs, 'norm', args=(0, 1.0))
        textstr = (f'K-S Test:\nstat={ks_stat:.4f}\np={ks_p:.2e}\n'
                   f'Mean={patch_outputs.mean():.4f}\n'
                   f'Scale={estimated_sigma:.4f}\n'
        ...
        stats.probplot(sample_data, dist=stats.norm(loc=0, scale=1.0), plot=ax4)
    ...
    for batch_idx, (images, _) in enumerate(data_loader):
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            logger.info(
                ...
                f"Sigma: {avg_stats['output_sigma']:.4f} | "
...
class PerPatchNormalize(object):
    """
    自定义变换：对每个图像单独计算mean和std，然后标准化到mean=0, std=1。
    输入：tensor (C, H, W)，输出：标准化后的tensor。
    """
...
def main(args):
    ...
    seed = 64  # 可以从 args 添加 --seed 参数来配置
    np.random.seed(seed)
    ...
    mine_model.load_state_dict(torch.load('./mine_weights_16.pth'))
    mine_model.eval()
...
if __name__ == "__main__":
    ...
    # Model args
    parser.add_argument("--hidden-dim", type=int, default=24, help="Hidden dimension for MLP encoder")
    # Loss weights
