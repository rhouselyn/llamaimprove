...
class GaussianKLLoss(nn.Module):
    """
    损失函数：
    只保留原始KL散度损失（编码分布与标准高斯的KL散度）
    """

    def __init__(self, weight_original=1.0):
        super().__init__()
...
def plot_comparison(before_data, after_data, save_path, epoch_info=""):
    ...
    for idx, (phase, color, label) in enumerate(zip(phases, colors, phase_labels)):
        ...
        ax3.hist(patch_outputs, bins='auto', density=True, alpha=0.7,
...
def main(args):
    ...
    logger.info(f"Loss weights - Original KL: {args.weight_original}")
