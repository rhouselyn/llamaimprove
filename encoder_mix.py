...
class GaussianKLLoss(nn.Module):
    ...
    2. 新增：基于STE的正值个数估计的混合损失（KL + PMF L1）
    """

    def __init__(self, weight_original=1.0, weight_count=1.0, distributed=False):
        super().__init__()
        self.weight_original = weight_original  # 原始KL散度权重
        self.weight_count = weight_count  # 第二项损失权重
        self.distributed = distributed
        self.encoding_size = 64
        self.target_pmf = torch.tensor(stats.binom.pmf(np.arange(self.encoding_size + 1), self.encoding_size, 0.5), dtype=torch.float32)
        self.mix_ratio = 1.0  # 初始为1.0，全KL，逐渐减小到0.0，全PMF L1
    ...
    def forward(self, encodings):
        ...
        # 计算经验PMF（直方图）
        approx_counts_int = all_approx_counts.round().long().clamp(0, encoding_size)
        counts = torch.bincount(approx_counts_int, minlength=encoding_size + 1)  # [65]
        ...
        total_N = 2 * N  # 因为拼接了2倍
        if self.distributed:
            total_N_tensor = torch.tensor([total_N], device=encodings.device)
            dist.all_reduce(total_N_tensor)
            total_N = total_N_tensor.item()

        avg_pmf = counts.float() / total_N

        # 计算目标PMF
        target_pmf = self.target_pmf.to(encodings.device)

        # 计算PMF的L1距离（逐项绝对差求和）
        pmf_loss = torch.abs(avg_pmf - target_pmf).sum()

        # 混合第二项损失
        second_loss = self.mix_ratio * kl_loss_count + (1 - self.mix_ratio) * pmf_loss

        # ===== 3. 总损失 =====
        total_loss = (self.weight_original * kl_loss_original +
                      self.weight_count * second_loss)
        ...
        stats_dict = {
            ...
            'kl_loss_count': second_loss.item(),  # 报告混合后的第二项损失
            'pmf_loss': pmf_loss.item(),  # 新增：PMF L1损失
            # 真实统计
...
def plot_comparison(before_data, after_data, save_path, epoch_info=""):
    ...
    for idx, (phase, color, label) in enumerate(zip(phases, colors, phase_labels)):
        ...
        ax2.hist(estimated_counts, bins='auto', density=True, alpha=0.7,
    ...
    for batch_idx, (images, _) in enumerate(data_loader):
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            logger.info(
                ...
                f"PMF_L1: {avg_stats['pmf_loss']:.6f} | "
...
def main(args):
    ...
    for epoch in range(args.epochs):
        # 根据epoch调整mix_ratio：从1.0 (全KL) 线性过渡到0.0 (全PMF L1)
        if args.epochs > 1:
            mix_ratio = max(0.0, 1.0 - (epoch / (args.epochs - 1.0)))
        else:
            mix_ratio = 0.0
        criterion.mix_ratio = mix_ratio
        logger.info(f"Epoch {epoch + 1}: Setting mix_ratio to {mix_ratio:.2f} (KL: {mix_ratio:.2f}, PMF_L1: {1 - mix_ratio:.2f})")
