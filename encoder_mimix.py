...
class MINELoss(nn.Module):
    """
    MINE 损失模块，使用提供的 Net 作为统计网络 T(x, y)。
    我们将 encodings [B, 64] 随机置换后拆分成 x [B, 32] 和 y [B, 32]，直接计算所有维度的整体 T(x, y)。
    这避免了逐对卷积，而是同时处理所有维度（通过 Net 的向量输入）。
    Net 的输入维度已泛化为 32（encoding_dim // 2），以匹配 64 维编码。
    """
    def __init__(self, encoding_dim=64):
        super().__init__()
        self.encoding_dim = encoding_dim
        self.half_dim = encoding_dim // 2

        # 使用提供的 Net 结构，但将输入维度从 7 泛化为 32
        class Net(nn.Module):
            def __init__(self, half_dim, hidden_dim):
                super(Net, self).__init__()
                self.fc1 = nn.Linear(half_dim, hidden_dim)  # 从 7 改为 32
                self.fc2 = nn.Linear(half_dim, hidden_dim)  # 从 7 改为 32
        ...
        self.T = Net(self.half_dim, self.encoding_dim // 4)  # hidden_dim 可以调整
    ...
    def forward(self, encodings):
        ...
        """
        B, D = encodings.shape
        assert D == self.encoding_dim

        # 维度随机置换，确保随机性
        perm_idx = torch.randperm(D, device=encodings.device)
        encodings_perm = encodings[:, perm_idx]  # [B, 64]

        # 拆分成两组作为 x 和 y（直接同时处理所有维度）
        x_joint = encodings_perm[:, :self.half_dim]  # [B, 32]
        y_joint = encodings_perm[:, self.half_dim:]  # [B, 32]

        # ========== 联合分布 ==========
        T_joint = self.T(x_joint, y_joint).squeeze()  # [B]

        # ========== 边缘分布 ==========
        # 在 batch 维度打乱 y（破坏联合分布）
        batch_perm = torch.randperm(B, device=encodings.device)
        y_marginal = y_joint[batch_perm]  # [B, 32]

        T_marginal = self.T(x_joint, y_marginal).squeeze()  # [B]
...
class ImprovedGaussianKLLoss(nn.Module):
    ...
    3. 新增：MINE 互信息损失（使用新的 MINELoss 模块，直接同时计算所有维度）
    （已移除基于STE的正值个数估计的Wasserstein-1距离损失）
    """
    def __init__(self, weight_original=1.0, weight_count=1.0, weight_mi=0.1, distributed=False):
        super().__init__()
        self.weight_original = weight_original  # 原始KL散度权重
        self.weight_count = weight_count  # 正值个数KL散度权重（保留但未使用）
        self.weight_mi = weight_mi  # MINE 互信息权重
        self.distributed = distributed
        self.encoding_size = 64

        # 使用新的 MINELoss 模块（替换了逐对卷积，直接同时计算所有维度）
        self.mine = MINELoss(encoding_dim=64)  # hidden_dim 可以调整
    ...
    def forward(self, encodings, current_epoch=None, total_epochs=None):
        ...
        stats_dict = {
            ...
            'mi_loss': mi_loss.item(),  # 新增 MINE 统计
            # 真实统计
    ...
    for batch_idx, (images, _) in enumerate(data_loader):
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            logger.info(
                ...
                f"MI_loss: {avg_stats.get('mi_loss', 0.0):.6f} | "  # 新增 MI 日志
...
def main(args):
    ...
    criterion = ImprovedGaussianKLLoss(
        ...
        weight_mi=args.weight_mi if hasattr(args, 'weight_mi') else 0.1,  # 如果命令行未指定，默认 0.1
        distributed=args.distributed
    ).to(device)

    logger.info(f"Loss weights - Original KL: {args.weight_original}, Count KL: {args.weight_count}, MI: {criterion.weight_mi}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weight-mi", type=float, default=1.0,
                        help="Weight for MINE mutual information loss")  # 新增 MI 权重参数
